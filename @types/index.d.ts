...
export type Channel = 'newitems_go'
    | 'history_go'
    | 'additem_go'
    | 'itemout_new_go'
    | 'itemstatus_go'
    | 'money'
    | 'webnotify';
export type Channels = { [channel in 'NEW_ITEMS_GO'
    | 'HISTORY_GO'
    | 'ADDITEM_GO'
    | 'ITEMOUT_NEW_GO'
    | 'ITEMSTATUS_GO'
...
export type TMSocketCallback = (data: TMSocketData) => void;
...
export type Currencies = { [currency in Currency]: Currency; }
...
export interface PriceInstancesResponse extends BaseResponse {
    readonly time: number;
    readonly currency: Currency;
    readonly items: { [key: string]: PriceInstanceItem; };
}
...
export interface BuyListInfoByCustomId extends BaseResponse {
    readonly data: { [key: string]: BuyInfo };
}
